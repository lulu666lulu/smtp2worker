...
推荐用 Docker Compose。它会读取当前目录的 `.env`，并把 SMTP 端口映射到宿主机：
...
docker run --rm -p 2525:2525 ^
  --env-file .env ^
  -e SMTP_LISTEN_HOST=0.0.0.0 ^
  smtp2worker
```

注意：如果 `.env` 里保留 `SMTP_LISTEN_HOST=127.0.0.1`，容器内会只监听容器自己的 loopback，宿主机连不到。所以 Docker 场景下需要像上面那样覆盖为 `0.0.0.0`；`compose.yaml` 已经帮你处理了。
